...
class Eavesdropper(Process):
    ...
    # def listen(self): REMOVE IF NOT REQUIRED
    #     message = self.incomming_connection.listen()
    #     if message != None:
    ...
    def transfer(self): # NOTE Asynchronous now, rewrite when done to sync
        ...
        if len(self.difference_lists(self.spoof_B.message_list, self.message_list)) > 0: # if there is a difference between the lists
            # Grab intel Fake Bob learned about
            transfer_message = self.spoof_B.message_list[-1]
            ...
            if type(transfer_message) != type(self.spoof_B.public_key):
                self.spoof_A.message_list.append(transfer_message)
